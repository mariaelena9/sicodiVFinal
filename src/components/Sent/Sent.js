...
class Sent extends Component {
    ...
    getSentCorrespondence() {
        axios.get(`http://localhost:3000/api/correspondence/getSent/${localStorage.getItem("idusuario")}`).then(res => {
    ...
    handleFilter = (event) => {
        ...
        axios.get(`http://localhost:3000/api/correspondence/filterSent/${event.target.value}`).then(res => {
            this.setState({ correspondencias: res.data });
            this.setState({ corresFiltradas: res.data });
        }).catch(error => {
            console.log(error.message);
